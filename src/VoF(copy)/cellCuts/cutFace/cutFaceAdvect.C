...
/* Temporarily taken out
    ...
    volScalarField::Boundary& abf = const_cast<volScalarField::Boundary&>(alpha1_.boundaryField());
        ...
        forAll(patches, patchi)
        {
            ...
            if (patchName=="bottomRemainder" || patchName=="inlet") //isA<alphaContactAngleTwoPhaseFvPatchScalarField>(abf[patchi]))
            {
                ...
                forAll(patches[patchi], facei) 
                {
                    ...
                    const label& gIdFacei = boundaryMesh[patchi].start() + facei;
                    if (faceI == gIdFacei)
                    {
    ...
    //Info << "out of insoection" << nl;

    // forAll(patches, patchi)
    // {
    //     const word & patchName = patches[patchi].name(); // Boundary patch name
    //     if (patchName=="bottomRemainder") //isA<alphaContactAngleTwoPhaseFvPatchScalarField>(abf[patchi]))
    //     {
    //         label patchID = boundaryMesh.findPatchID(patchName);
    //         forAll(patches[patchi], facei) 
    //         {
    //             const label & face = boundaryMesh[patchi].start() + facei; // Face index
    //             //Checking if the faceI (the current face for advction) belongs to patch with DCA BC
    //             //Then we will check CL for the face
    //             Info << " face labels check " << face << " - " << faceI << " on patch " << patchName << nl;
    //             if (face==faceI)
    //             {
    //                 isRobinBCFace = true;
    //             }
    ...
    //                 const auto& meshPoints = mesh_.points();
    //                 const auto& meshFaces = mesh_.faces();
    //                 const auto& thisFace = meshFaces[patches[patchi].start() + faceI];
    //                 Info << " This face " << thisFace << nl;
    //                 // Get face points. 
    //                 for(auto pointI = 0; pointI < (thisFace.size() - 1); ++pointI)
    //                 {
    ...
    //                 }
    //             }
    //             if (isRobinBCFace && isAContactLine)
    //             {
    //                 const vectorField nf(patches[patchi].nf()); 
    //                 scalar thetaA = 110;
    //                 scalar thetaR = 60;
    //                 scalar contactAngle = acos((n0 & nf[faceI]) / (mag(n0) * mag(nf[faceI]))) * 180/M_PI;
    ...
    if ((mag(Un0) > 1e-12) && (!insideHysteresis)) // Note: tolerances
    {
    ...
    else
    {
        if (insideHysteresis)
        {
            Info << "Before: Hysteresis: For faceI " << faceI << " with subFaceArea " << subFaceArea() << " and alphaf " << mag(subFaceArea() / magSf) << nl;
            // Un0 is almost zero and isoFace is treated as stationary
            calcSubFace(faceI, -n0, x0);
            const scalar alphaf = mag(subFaceArea() / magSf);
            Info << "After: Hysteresis: For faceI " << faceI << " with subFaceArea " << subFaceArea() << " and alphaf " << mag(subFaceArea() / magSf) << nl;
