...
Description
    ...
    employs the empirical Bracke model for dynamic contact angle 
    ((taken from Dirk Drunding and Anja Lippert dissertation)) i.e.,
    
    // theta = arccos (cos\theta_e - 2*sqrt(Ca)*(cos\theta_e + 1)) 
...
#include "dynamicAlphaContactAngleBrackeFvPatchScalarField.H"
...
bool Foam::dynamicAlphaContactAngleBrackeFvPatchScalarField::
hasContactLine
(
    label faceI
) const
...
Foam::dynamicAlphaContactAngleBrackeFvPatchScalarField::
dynamicAlphaContactAngleBrackeFvPatchScalarField
(
    const fvPatch& p,
...
dynamicAlphaContactAngleBrackeFvPatchScalarField
(
    const dynamicAlphaContactAngleBrackeFvPatchScalarField& gcpsf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
Foam::dynamicAlphaContactAngleBrackeFvPatchScalarField::theta
(
    ...
    {   
        if (hasContactLine(faceI))
        {
            ...
            thetaf[faceI] = min( 180 / constant::mathematical::pi *     
                                acos(cos(theta0_) - 2*sqrt(mag(Ca[faceI]))*(cos(theta0_) + 1)), 
                            scalar(180));
...
void Foam::dynamicAlphaContactAngleBrackeFvPatchScalarField::write(Ostream& os) const
{
