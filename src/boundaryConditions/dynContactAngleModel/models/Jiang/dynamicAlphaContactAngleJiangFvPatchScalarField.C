...
Description
    ...
    employs the empirical Jiang model for dynamic contact angle 
    ((taken from Dirk Drunding and Anja Lippert dissertation)) i.e.,
    
    // theta = arccos (cos\theta_e - tanh(4.96*Ca^(0.702))(cos\theta_e + 1)) 
...
bool Foam::dynamicAlphaContactAngleJiangFvPatchScalarField::
hasContactLine
(
    label faceI
) const
    ...
    {   
        if (hasContactLine(faceI))
        {
            ...
            thetaf[faceI] = min( 180 / constant::mathematical::pi *     
                                acos(cos(theta0_) - tanh(4.96*pow(mag(Ca[faceI]),0.702))*(cos(theta0_) + 1)), 
                            scalar(180));
